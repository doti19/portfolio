...
<PrismicLink field={linkField} class={`group text-slate-900 relative flex w-fit items-center justify-center overflow-hidden rounded-md border-2 border-slate-900 bg-slate-50 px-4 py-2 font-bold transition-transform eas-out hover:scale-105 ${className}`}>
    ...
    <span
        ...
        <span class="relative flex items-center justify-center gap-2">{label}
            {#if showIcon }
            <IconArrow />
            {/if}
